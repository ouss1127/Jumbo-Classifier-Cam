...
CAMERA_STATUS_FAILED = -1   # Operation failure
CAMERA_STATUS_INTERNAL_ERROR = -2   # Internal error
CAMERA_STATUS_UNKNOW = -3   # Unknown error
CAMERA_STATUS_NOT_SUPPORTED = -4   # This feature is not supported
CAMERA_STATUS_NOT_INITIALIZED = -5   # Initialization not completed
CAMERA_STATUS_PARAMETER_INVALID = -6   # Invalid parameters
CAMERA_STATUS_PARAMETER_OUT_OF_BOUND = -7   # Parameters out of bounds
CAMERA_STATUS_UNENABLED = -8   # Not Enabled
CAMERA_STATUS_USER_CANCEL = -9   # The user manually canceled, for example, the roi panel clicked cancel and returned
CAMERA_STATUS_PATH_NOT_FOUND = -10  # The corresponding path was not found in the registry
CAMERA_STATUS_SIZE_DISMATCH = -11  # Mismatch between the length of the acquired image data and the defined dimensions
...
#AIA's standard compatible error codes
...
# Camera operation exception messages
...
# The camera's device information, read-only information, do not modify
...
# Resolution setting range of the camera
...
#Description of the resolution of the camera
...
class tSdkExpose(MvStructure):
	_fields_ = [
		...
		("uiExposeTimeMax", c_uint),   #The maximum value of exposure time in manual mode, unit:line
	]
...
#Transmission sub-packet size description (mainly valid for network cameras)
...
#Image data format output by the camera
...
#Enabling information for ISP modules
...
# Define integrated device description information that can be used to dynamically build the UI
class tSdkCameraCapbility(MvStructure):
	_fields_ = [
		("pTriggerDesc", POINTER(tSdkTrigger)),
		("iTriggerDesc", c_int),	#The number of trigger patterns, i.e. the size of the pTriggerDesc array
		("pImageSizeDesc", POINTER(tSdkImageResolution)),
		("iImageSizeDesc", c_int),	#The number of preset resolutions, i.e. the size of the pImageSizeDesc array 
		("pClrTempDesc", POINTER(tSdkColorTemperatureDes)),
		("iClrTempDesc", c_int),	#Number of preset color temperature
		("pMediaTypeDesc", POINTER(tSdkMediaType)),
		("iMediaTypeDesc", c_int),	#The number of types of image formats output by the camera, i.e., the size of the pMediaTypeDesc array.
		("pFrameSpeedDesc", POINTER(tSdkFrameSpeed)), #Adjustable frame speed type, corresponding to three speed settings on the interface: normal, high speed and super
		("iFrameSpeedDesc", c_int), #The number of adjustable frame speed types, i.e. the size of the pFrameSpeedDesc array.
		("pPackLenDesc", POINTER(tSdkPackLength)), #Transmission packet length, generally used for network devices
		("iPackLenDesc", c_int), #The number of transmission sub-packet lengths available, i.e. the size of the pPackLenDesc array. 
		("iOutputIoCounts", c_int),        #Number of programmable output IOs
		("iInputIoCounts", c_int),         #Number of programmable input IOs
		("pPresetLutDesc", POINTER(tSdkPresetLut)), #LUT table for camera presets
		("iPresetLut", c_int),             #The number of LUT tables preset by the camera, i.e., the size of the pPresetLutDesc array
		("iUserDataMaxLen", c_int),        #Indicates the maximum length of the user data area used to store user data in this imager. A value of 0 indicates none.
		("bParamInDevice", c_int),         #Indicates whether the device supports reading and writing parameter groups from the device. 1 is supported, 0 is not supported.
		...
		("pAeAlmHdDesc", POINTER(tSdkAeAlgorithm)),#Hardware automatic exposure algorithm description, NULL that does not support hardware automatic exposure
		("iAeAlmHdDesc", c_int),           #Hardware auto-exposure algorithm number, to 0 that does not support hardware auto-exposure
		("pBayerDecAlmSwDesc", POINTER(tSdkBayerDecodeAlgorithm)),#Description of the algorithm for converting software Bayer to RGB data
		("iBayerDecAlmSwDesc", c_int),     #Number of algorithms to convert software Bayer to RGB data
		("pBayerDecAlmHdDesc", POINTER(tSdkBayerDecodeAlgorithm)),#Description of the algorithm for converting hardware Bayer to RGB data, NULL means not supported
		("iBayerDecAlmHdDesc", c_int),     #Number of algorithms for converting hardware Bayer to RGB data, 0 means not supported

		# Definition of the adjustment range of image parameters for dynamic UI construction
		("sExposeDesc", tSdkExpose),      #Range value of exposure
		("sResolutionRange", tSdkResolutionRange), #Resolution Range Description  
		("sRgbGainRange", tRgbGainRange),    #Image digital gain range description  
		("sSaturationRange", tSaturationRange), #Saturation range description  
		("sGammaRange", tGammaRange),      #Gamma range description  
		("sContrastRange", tContrastRange),   #Contrast range description
		("sSharpnessRange", tSharpnessRange),  #Sharpening range description  
		("sIspCapacity", tSdkIspCapacity),     #ISP Capability Description
	]
...
class tSdkFrameHead(MvStructure):
	_fields_ = [
		...
		("uBytes", c_uint),           # Total bytes of image data
		("iWidth", c_int),            # Image height
		("iHeight", c_int),           # Image width
		("iWidthZoomSw", c_int),      # The width of the software scaling, this variable is set to 0 for images that do not require software cropping.
		("iHeightZoomSw", c_int),     # The height of the software scaling, this variable is set to 0 for images that do not require software cropping.
		("bIsTrigger", c_int),        # Indicates if the frame is trigger 
		("uiTimeStamp", c_uint),      # Acquisition time of the frame in 0.1 ms 
		("uiExpTime", c_uint),        # The exposure value of the current image in microseconds us
		("fAnalogGain", c_float),     # Analog gain multiplier of the current image
		("iGamma", c_int),            # Gamma setting for the frame, valid only when the LUT mode is dynamic parameter generation, -1 in the rest of the modes
		("iContrast", c_int),         # The contrast setting of the frame is valid only when the LUT mode is the state parameter generation, and -1 in the rest of the modes
		("iSaturation", c_int),       # The saturation setting for this frame, which is meaningless for black and white cameras, is 0
		("fRgain", c_float),          # The red digital gain multiplier for this frame image processing, which is meaningless for black and white cameras, is 1
		("fGgain", c_float),          # The green digital gain multiplier for this frame image processing, which is meaningless for black and white cameras, is 1
		("fBgain", c_float),          # The blue digital gain multiplier for this frame image processing, which is meaningless for black and white cameras, is 1
	]
...
# Store the error code returned by the last SDK call
def GetLastError():
	try:
